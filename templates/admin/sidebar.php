<?php


?>

<!-- sidebar -->
<nav id="sidebar">
    <ul class="sidebar-items sidebar-flexbox-col">

        <li class="sidebar-item sidebar-flexbox-left">
            <a href="/" class="sidebar-item-inner sidebar-flexbox-left" href="/">
                <div class="sidebar-item-inner-icon-wrapper sidebar-flexbox">
                    <ion-icon name="home-outline">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
                            <path fill="currentColor" d="M20 7.093v-5.093h-3v2.093l3 3zm4 5.907l-12-12-12 12h3v10h7v-5h4v5h7v-10h3zm-5 8h-3v-5h-8v5h-3v-10.26l7-6.912 7 6.99v10.182z" />
                        </svg>
                    </ion-icon>
                </div>
                <span class="link-text">Vista previa</span>
            </a>
        </li>
        <li class="sidebar-item sidebar-flexbox-left">
            <a class="sidebar-item-inner sidebar-flexbox-left" href="/admin/products">
                <div class="sidebar-item-inner-icon-wrapper sidebar-flexbox">
                    <ion-icon name="folder-open-outline">
                        <svg fill="currentColor" width="24" height="24" xmlns="http://www.w3.org/2000/svg" fill-rule="evenodd" clip-rule="evenodd">
                            <path fill="currentColor" d="M23 6.066v12.065l-11.001 5.869-11-5.869v-12.131l11-6 11.001 6.066zm-21.001 11.465l9.5 5.069v-10.57l-9.5-4.946v10.447zm20.001-10.388l-9.501 4.889v10.568l9.501-5.069v-10.388zm-5.52 1.716l-9.534-4.964-4.349 2.373 9.404 4.896 4.479-2.305zm-8.476-5.541l9.565 4.98 3.832-1.972-9.405-5.185-3.992 2.177z" />
                        </svg>

                    </ion-icon>
                </div>
                <span class="link-text">Productos</span>
            </a>
        </li>
        <li class="sidebar-item sidebar-flexbox-left">
            <a class="sidebar-item-inner sidebar-flexbox-left" href="/admin/dashboard">
                <div class="sidebar-item-inner-icon-wrapper sidebar-flexbox">
                    <ion-icon name="pie-chart-outline">
                        <svg fill="currentColor" width="24" height="24" clip-rule="evenodd" fill-rule="evenodd" stroke-linejoin="round" stroke-miterlimit="2" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            <path fill="currentColor" d="m21 4c0-.478-.379-1-1-1h-16c-.62 0-1 .519-1 1v16c0 .621.52 1 1 1h16c.478 0 1-.379 1-1zm-16.5.5h15v15h-15zm13.5 9c0-.276-.224-.5-.5-.5h-4c-.276 0-.5.224-.5.5v4c0 .276.224.5.5.5h4c.276 0 .5-.224.5-.5zm-10.061 1.99-1.218-1.218c-.281-.281-.282-.779 0-1.061s.78-.281 1.061 0l1.218 1.218 1.218-1.218c.281-.281.779-.282 1.061 0s.281.78 0 1.061l-1.218 1.218 1.218 1.218c.281.281.282.779 0 1.061s-.78.281-1.061 0l-1.218-1.218-1.218 1.218c-.281.281-.779.282-1.061 0s-.281-.78 0-1.061zm8.561-.99v2h-2v-2zm-7.5-8.5c1.656 0 3 1.344 3 3s-1.344 3-3 3-3-1.344-3-3 1.344-3 3-3zm9 5.25c0-.399-.353-.75-.75-.75-1.153 0-2.347 0-3.5 0-.397 0-.75.351-.75.75s.353.75.75.75h3.5c.397 0 .75-.351.75-.75zm-9-3.75c-.828 0-1.5.672-1.5 1.5s.672 1.5 1.5 1.5 1.5-.672 1.5-1.5-.672-1.5-1.5-1.5zm9 1.5c0-.399-.353-.75-.75-.75-1.153 0-2.347 0-3.5 0-.397 0-.75.351-.75.75s.353.75.75.75h3.5c.397 0 .75-.351.75-.75zm0-2.25c0-.399-.353-.75-.75-.75-1.153 0-2.347 0-3.5 0-.397 0-.75.351-.75.75s.353.75.75.75h3.5c.397 0 .75-.351.75-.75z" fill-rule="nonzero" />
                        </svg>
                    </ion-icon>
                </div>
                <span class="link-text">Dashboard</span>
            </a>
        </li>
        <li class="sidebar-item sidebar-flexbox-left">
            <a class="sidebar-item-inner sidebar-flexbox-left" href="/admin/sells">
                <div class="sidebar-item-inner-icon-wrapper sidebar-flexbox">
                    <ion-icon name="sells-outline">
                        <svg fill="currentColor" width="24" height="24" xmlns="http://www.w3.org/2000/svg" fill-rule="evenodd" clip-rule="evenodd">
                            <path fill="currentColor" d="M6.72 20.492c1.532.956 3.342 1.508 5.28 1.508 1.934 0 3.741-.55 5.272-1.503l1.24 1.582c-1.876 1.215-4.112 1.921-6.512 1.921-2.403 0-4.642-.708-6.52-1.926l1.24-1.582zm-5.317-1.33h-1.403c0-1.104.895-2 2-2h2c.53 0 1.039.211 1.414.586s.586.884.586 1.414h-1.403c-.196-.606-.755-.726-1.597-.726s-1.4.115-1.597.726zm10.559-11.162c-3.037 0-5.5 2.462-5.5 5.5 0 3.037 2.463 5.5 5.5 5.5s5.5-2.463 5.5-5.5c0-3.038-2.463-5.5-5.5-5.5zm7.441 11h-1.403c0-1.105.895-2 2-2h2c.53 0 1.039.211 1.414.586s.586.883.586 1.414h-1.403c-.196-.606-.755-.726-1.597-.726s-1.4.115-1.597.726zm-7.441-9.5c2.346 0 4 1.654 4 4s-1.654 4-4 4-4-1.654-4-4 1.654-4 4-4zm-8.962 2.662c1.242 0 2.25 1.008 2.25 2.25s-1.008 2.25-2.25 2.25-2.25-1.008-2.25-2.25 1.008-2.25 2.25-2.25zm18-.162c1.242 0 2.25 1.008 2.25 2.25s-1.008 2.25-2.25 2.25-2.25-1.008-2.25-2.25 1.008-2.25 2.25-2.25zm-8.75-1.5v.509c.362.01.768.073 1.221.21l-.181.824c-.384-.135-.808-.257-1.221-.232-.745.043-.81.688-.291.958.856.402 1.972.7 1.972 1.772.001.859-.672 1.316-1.5 1.432v.527h-.5v-.499c-.518-.009-1.053-.133-1.5-.364l.227-.822c.478.186 1.114.383 1.613.27.574-.13.692-.721.057-1.005-.466-.217-1.889-.403-1.889-1.621 0-.682.519-1.292 1.492-1.425v-.534h.5zm-9.25 3.012c.497 0 .9.403.9.9s-.403.9-.9.9-.9-.403-.9-.9.403-.9.9-.9zm18-.162c.497 0 .9.403.9.9s-.403.9-.9.9-.9-.403-.9-.9.403-.9.9-.9zm-13-12.666v2.149c-2.938 1.285-5.141 3.942-5.798 7.158l-2.034-.003c.732-4.328 3.785-7.872 7.832-9.304zm8 0c4.047 1.432 7.1 4.976 7.832 9.304l-2.034.003c-.657-3.216-2.86-5.873-5.798-7.158v-2.149zm-5.597 6.316h-1.403c0-1.105.895-2 2-2h2c.53 0 1.039.211 1.414.586s.586.883.586 1.414h-1.403c-.196-.606-.755-.726-1.597-.726s-1.4.115-1.597.726zm1.597-7c1.242 0 2.25 1.008 2.25 2.25s-1.008 2.25-2.25 2.25-2.25-1.008-2.25-2.25 1.008-2.25 2.25-2.25zm0 1.35c.497 0 .9.403.9.9s-.403.9-.9.9-.9-.403-.9-.9.403-.9.9-.9z" />
                        </svg>

                    </ion-icon>
                </div>
                <span class="link-text">Ventas</span>
            </a>
        </li>
        <li class="sidebar-item sidebar-flexbox-left">
            <a class="sidebar-item-inner sidebar-flexbox-left" href="/admin/stats">
                <div class="sidebar-item-inner-icon-wrapper sidebar-flexbox">
                    <ion-icon name="stats-outline">
                        <svg fill="currentColor" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                            <path fill="currentColor" d="M24 3.875l-6 1.221 1.716 1.708-5.351 5.358-3.001-3.002-7.336 7.242 1.41 1.418 5.922-5.834 2.991 2.993 6.781-6.762 1.667 1.66 1.201-6.002zm0 17.125v2h-24v-22h2v20h22z" />
                        </svg>
                    </ion-icon>
                </div>
                <span class="link-text">Estadisticas</span>
            </a>
        </li>
        <li class="sidebar-item sidebar-flexbox-left">
            <a class="sidebar-item-inner sidebar-flexbox-left" href="/admin/settings">
                <div class="sidebar-item-inner-icon-wrapper sidebar-flexbox">
                    <ion-icon name="settings-outline">
                        <svg fill="currentColor" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                            <path fill="currentColor" d="M12 9c.552 0 1 .449 1 1s-.448 1-1 1-1-.449-1-1 .448-1 1-1zm0-2c-1.657 0-3 1.343-3 3s1.343 3 3 3 3-1.343 3-3-1.343-3-3-3zm-9 4c-1.657 0-3 1.343-3 3s1.343 3 3 3 3-1.343 3-3-1.343-3-3-3zm18 0c-1.657 0-3 1.343-3 3s1.343 3 3 3 3-1.343 3-3-1.343-3-3-3zm-9-6c.343 0 .677.035 1 .101v-3.101c0-.552-.447-1-1-1s-1 .448-1 1v3.101c.323-.066.657-.101 1-.101zm9 4c.343 0 .677.035 1 .101v-7.101c0-.552-.447-1-1-1s-1 .448-1 1v7.101c.323-.066.657-.101 1-.101zm0 10c-.343 0-.677-.035-1-.101v3.101c0 .552.447 1 1 1s1-.448 1-1v-3.101c-.323.066-.657.101-1 .101zm-18-10c.343 0 .677.035 1 .101v-7.101c0-.552-.447-1-1-1s-1 .448-1 1v7.101c.323-.066.657-.101 1-.101zm9 6c-.343 0-.677-.035-1-.101v7.101c0 .552.447 1 1 1s1-.448 1-1v-7.101c-.323.066-.657.101-1 .101zm-9 4c-.343 0-.677-.035-1-.101v3.101c0 .552.447 1 1 1s1-.448 1-1v-3.101c-.323.066-.657.101-1 .101z" />
                        </svg>
                    </ion-icon>
                </div>
                <span class="link-text">Configuracion</span>
            </a>
        </li>
    </ul>
</nav>

<style>
    :root {
        --primary: 237, 94%, 81%;
        --background: 266, 16%, 92%;
        --background-secondary: 256, 12%, 12%;
        --background-secondary-dark: 256, 10%, 10%;
        --background-secondary-light: 257, 11%, 16%;
        --text-primary: 0, 0%, 0%;
        /* Colors */
        --black: 0, 0%, 0%;
        --white: 0, 0%, 100%;
        --quite-gray: 0, 0%, 50%;
        --grooble: 10, 28%, 93%;
        /* Sizes */
        --heading-large: 5.6rem;
        --heading-medium: 3.6rem;
        --heading-small: 2.4rem;
        --paragraph: 1.11rem;
        --sidebar-buttons: 2.4rem;
        /* misc */
        --transition-main: .175, .685, .32;
        /* Fonts */
        --font-main: "system-ui";
    }

    body,
    html {
        padding: 0;
        margin: 0;
        min-height: 100vh;
        min-width: 100vw;

        font-family: system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, 'Open Sans', 'Helvetica Neue', sans-serif;
    }

    main {
        display: flex;
        flex-direction: column;
        width: 100%;
        height: 100%;
    }

    .sidebar-flexbox {
        display: flex;
        justify-content: center;
        align-items: center;
    }

    .sidebar-flexbox-left {
        display: flex;
        justify-content: flex-start;
        align-items: center;
    }

    .sidebar-flexbox-right {
        display: flex;
        justify-content: flex-end;
        align-items: center;
    }

    .sidebar-flexbox-col {
        display: flex;
        justify-content: center;
        flex-direction: column;
        align-items: center;
    }

    .sidebar-flexbox-col-left {
        display: flex;
        justify-content: flex-start;
        flex-direction: column;
        align-items: flex-start;
    }

    .sidebar-flexbox-col-left-ns {
        display: flex;
        justify-content: center;
        flex-direction: column;
        align-items: flex-start;
    }

    .sidebar-flexbox-col-right {
        display: flex;
        justify-content: flex-end;
        flex-direction: column;
        align-items: flex-end;
    }

    .sidebar-flexbox-col-start-center {
        display: flex;
        justify-content: flex-start;
        flex-direction: column;
        align-items: center;
    }

    .sidebar-flexbox-space-bet {
        display: flex;
        justify-content: space-between;
        align-items: center;
    }

    .view-width {
        width: 70%;
    }

    #sidebar {
        top: 0;
        padding: 0;
        width: 4em;
        height: 100vh;
        /* position: fixed; */
        /* background-color: hsl(var(--background-secondary)); */
        background-color: #eeee;
        transition: width .35s cubic-bezier(var(--transition-main), 1);
        overflow-y: auto;
        overflow-x: hidden;
    }

    #sidebar:hover {
        width: 16em;
    }

    #sidebar::-webkit-scrollbar-track {
        background-color: hsl(var(--background-secondary));
    }

    #sidebar::-webkit-scrollbar {
        width: 8px;
        background-color: hsl(var(--background-secondary));
    }

    #sidebar::-webkit-scrollbar-thumb {
        background-color: hsl(var(--primary));
    }

    .sidebar-items {
        margin: 0;
        padding: 0;
        list-style-type: none;
    }

    .sidebar-logo {
        margin: 0 0 2em 0;
        width: 100%;
        height: 5em;
        background: hsl(var(--background-secondary-dark));
    }

    .sidebar-logo>.sidebar-item-inner {
        width: calc(5rem - 8px);
        display: flex;
        flex-direction: row;
        place-items: center;
        justify-content: center
    }

    .sidebar-logo>.sidebar-item-inner:hover {
        background-color: transparent;
    }

    .sidebar-logo>.sidebar-item-inner>svg {
        height: 2em;
        fill: hsl(var(--white));
    }

    .sidebar-item {
        padding: 0 .5em;
        width: 100%;
        cursor: pointer;
    }

    .sidebar-item-inner {
        padding: 1em 0;
        width: 100%;
        position: relative;
        color: hsl(var(--quite-gray));
        border-radius: .25em;
        text-decoration: none;
        transition: all .2s cubic-bezier(var(--transition-main), 1);
    }

    .sidebar-item-inner:hover {
        color: black;
        background: #ccc;
        box-shadow: 0 17px 30px -10px #ccc;
    }

    .sidebar-item-inner-icon-wrapper {
        width: calc(5rem - 1em - 8px);
        position: relative;
    }

    .sidebar-item-inner-icon-wrapper ion-icon {
        position: absolute;
        font-size: calc(var(--sidebar-buttons) - 1rem);
    }

    .link-text {
        margin: 0;
        width: 0;
        text-overflow: ellipsis;
        white-space: nowrap;
        transition: all .35s cubic-bezier(var(--transition-main), 1);
        overflow: hidden;
        opacity: 0;
    }

    #sidebar:hover .link-text {
        width: calc(100% - calc(5rem - 8px));
        opacity: 1;
    }

    #main {
        margin: 0 0 0 5em;
        min-height: 150vh;
    }

    #main>h2 {
        width: 80%;
        max-width: 80%;
    }

    #main>p {
        width: 80%;
        max-width: 80%;
    }

    ion-icon {
        color: #666;
    }

    ::selection {
        color: hsl(var(--white));
        background: hsla(var(--primary), .33);
    }

    ::-webkit-scrollbar-track {
        background-color: hsl(var(--background));
    }

    ::-webkit-scrollbar {
        width: 8px;
        background-color: hsl(var(--background));
    }

    ::-webkit-scrollbar-thumb {
        background-color: hsl(var(--primary));
    }

    @media only screen and (max-width: 1660px) {
        :root {
            /* Sizes */
            --heading-large: 5.4rem;
            --heading-medium: 3.4rem;
            --heading-small: 2.2rem;
        }
    }

    @media only screen and (max-width: 1456px) {
        :root {
            /* Sizes */
            --heading-large: 5.2rem;
            --heading-medium: 3.2rem;
            --heading-small: 2rem;
        }

        .view-width {
            width: 80%;
        }
    }

    @media only screen and (max-width: 1220px) {
        .view-width {
            width: 70%;
        }
    }

    @media only screen and (max-width: 1024px) {
        :root {
            /* Sizes */
            --heading-large: 5rem;
            --heading-medium: 3rem;
            --heading-small: 1.8rem;
        }

        .view-width {
            width: 75%;
        }
    }

    @media only screen and (max-width: 756px) {
        :root {
            /* Sizes */
            --heading-large: 4rem;
            --heading-medium: 2.6rem;
            --heading-small: 1.6rem;
            --paragraph: 1rem;
            --sidebar-buttons: 2.2rem;
        }

        .view-width {
            width: calc(100% - 5em);
        }
    }

    @media only screen and (max-width: 576px) {
        .view-width {
            width: calc(100% - 3em);
        }
    }

    @media only screen and (max-width: 496px) {}
</style>